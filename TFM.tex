...
To obtain topological descriptors from a grayscale image, we consider each pixel to be associated with an intensity value. This image can be interpreted as a function defined on a grid, in which the intensity values represent different levels of the structure. To analyze this image using TDA, we study how the structure changes when we progressively vary a threshold over the intensity values.
...
These features are represented using persistence diagrams (PDs), which describe the lifetime of each topological feature from its birth to its death: each point in the diagram with coordinates $(b_i, d_i)$ corresponds to a feature with birth $b_i$ and death $d_i$. These diagrams provide a compact summary of the topological structure of the image across all thresholds. Since PDs are not directly suitable for most machine learning models, they are usually transformed into a finite-dimensional representations. This project uses the following:

\begin{enumerate}
    \item \textbf{Betti curves:} show how many descriptors are alive at any given time. They can be compute for the topological descriptors of each homology class: in grayscale images: $H_0$ and $H_1$. These curves can be discretized  and fed as vectors to classifier models.\cite{tfg}
    \item \textbf{Landscapes:} given a persistence diagram, the landscapes can be computed by rotating it and laying it against the diagonal. Then, tent functions are computed for every point above the diagonal: lines paralel to the original axis that connect the point with the diagonal. The landscape functions can be discretized and fed as vectors to classifier models.\cite{tfg}
    \item \textbf{Silhouettes:} are a weighted average of tent functions with weights $w_i > 0$. A popular choice is $w_i = (d_i - b_i)^p$ where $(b_i, d_i)$ is each point of the persistence diagram and $p$ is a positive integer. Again, these functions can be discretized and fed as vectors to classifier models.\cite{tfg}
    \item \textbf{Persistence images (PIs):} the persistence diagram is converted into a persistence image by mapping each point into a continuous domain (usually transposing the persistence diagram) and applying a suitable probability distribution and weighting function, $f:\mathbb{R}^2 \to \mathbb{R}$, that must be zero along the horizontal axis, continuous and picewise differentiable\cite{persistence_images_original_paper}. The example in \cite[Image 1]{persistence_images_original_paper} illustrates this procedure.
\end{enumerate}
...
The integration of TDA can help address this limitation, providing complementary information about the shape and connectivity of data distributions.
...
Second, other topological descriptors coming from images can be combined with visual transformers via vector composition, and that has not been broadly adressed in the papers found.

Lastly, the computational cost and scalability of TDA components are not deeply analyzed in these works. Persistent homology can be expensive to compute, especially for high-resolution medical images, and its impact on training efficiency remains an open question.
